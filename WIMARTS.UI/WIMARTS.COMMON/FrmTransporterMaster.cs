...
{
    public partial class FrmTransporterMaster : Form
    { 
        ...
        public FrmTransporterMaster(UserMaster oCurUser)
        {
            InitializeComponent();
            curUser = oCurUser;
        ...
        public FrmTransporterMaster(UserMaster oCurUser, RedCRUDAS.eCRUDASState State)
        {
            InitializeComponent();
            curUser = oCurUser;
            eState = State;
        ...
        private void FrmTransporterMaster_Shown(object sender, EventArgs e)
        {
        ...
        private void FrmTransporterMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void txtCustName_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                epErrors.SetError(txtTransporterName, "Transporter Name name must be entered.");
        ...
        private TransporterDetails m_dbBusObj;

        private void InitializeRADUI()
        {
            bllMgr = new BLLManager();
            redGridControl1.dbBindingSrc.DataSource = typeof(TransporterDetails);
        ...
        {
            redGridControl1.SetBindingTXT(txtTransporterName, () => m_dbBusObj.TransporterName);
            ...
            redGridControl1.SetBindingTXT(TBX_Address, () => m_dbBusObj.Address);
        ...
        {
            System.Data.DataTable dt = bllMgr.TransporterDetailsBLL.GetTransporterDetailsDT();
            redGridControl1.LoadGridData(dt);
        }

        private bool NewRecord()
        {
            m_dbBusObj = new TransporterDetails();
                ...
                if (ValidateData() != false)
                {
                    ...
                    switch (redCRUDAS1.RADUI_State)
                    {
                        case RedRapidUI.RedCRUDAS.eCRUDASState.New:
                            ...
                            oCustID = bllMgr.TransporterDetailsBLL.AddTransporterDetails(m_dbBusObj);
                           Msg4User = "Record Added Successfully";
                            break;
                        case RedRapidUI.RedCRUDAS.eCRUDASState.Edit:

                            bllMgr.TransporterDetailsBLL.UpdateTransporterDetails(m_dbBusObj);
        ...
        private bool ValidateData()
        {
            try
            {
                if (txtTransporterName.Text.Trim() == "") { MessageBox.Show("Enter Customer's Legal Name", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation); txtTransporterName.Focus(); return false; }
        ...
        private bool DeleteRecord()
        {
            ...
            {
                int curSelectedRow = redGridControl1.DGSelectedRec;
                m_dbBusObj.TransporterID = redGridControl1.GetGridVal(0, curSelectedRow);

                if (MessageBox.Show("Are you sure to delete selected record?", this.Text, MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    if (m_dbBusObj != null)
                    {
                        bllMgr.TransporterDetailsBLL.RemoveTransporterDetails(m_dbBusObj);
                        LoadDBData();
                        curSelectedRow = (curSelectedRow < redGridControl1.DGTotalRec) ? curSelectedRow : redGridControl1.DGTotalRec - 1;
                        redGridControl1.dgMainData.Rows[curSelectedRow].Selected = true;
                        DisplayRecord(curSelectedRow);
        ...
        private bool DisplayRecord(int recordNum)
        {
            ...
            if (redGridControl1.dgMainData.RowCount > 0)
            {
                ...
                m_dbBusObj = bllMgr.TransporterDetailsBLL.GetTransporterDetails(redGridControl1.GetGridVal(0, recordNum));
                redGridControl1.dbBindingSrc.Add(m_dbBusObj);
               return true;
            }
            return false;
